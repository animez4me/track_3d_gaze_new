...
inline void sigint_handler(int s)
{
    stop = true;
}
...
public:
	...
	K2G(Processor p = CPU, bool mirror = false, bool channels3 = false, std::string serial = std::string()): mirror_(mirror), channels3_(channels3), listener_(libfreenect2::Frame::Color | libfreenect2::Frame::Ir | libfreenect2::Frame::Depth),
    ...
    bool getDepthByColor_(const cv::Point2f &point2d, cv::Point3f &point3d)
    {
//        registration_->getPointXYZ(&undistorted_, int(point2d.y), int(point2d.x), point3d.x, point3d.y, point3d.z);
        registration_->getPointXYZ(&undistorted_, point2d.y, point2d.x, point3d.x, point3d.y, point3d.z);
