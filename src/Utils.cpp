...
void drawFixation(cv::Mat image, cv::Point2f point)
{
	...
	cv::circle(image, point, radiuss, color, thick, lineType);
}
...
{
	cv::Mat P = cv::Mat::zeros(2, 3, CV_32F);

	rotation.copyTo(P.row(0).colRange(0, 3));
	translation.copyTo(P.row(1).colRange(0, 3));
...
{
	cv::Mat M = cv::Mat::zeros(3, 1, CV_32F);
...
cv::Point3f Mat_to_Point(cv::Mat point)
{
	...
	p.x = point.at<float>(0, 0);
	p.y = point.at<float>(1, 0);
	p.z = point.at<float>(2, 0);
...
pcl::PointXYZ Point_to_Cloud(cv::Point3f point)
{
	...
	cloud->points[0].x = point.x;
	cloud->points[0].y = point.y;
	cloud->points[0].z = point.z;
